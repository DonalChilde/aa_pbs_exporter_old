...
def get_page_contents(file_path: Path, start_page: int = 0, end_page: int = -1):
    ...
    with open(file_path, "rb") as file:
        ...
        if start_page > end_page:
            raise ValueError(
                f"{start_page} is bigger than {end_page}. Cannot count backwards."
            )

        for page_number in range(start_page, end_page):
            page = read_pdf.getPage(page_number)
            page_text = page.extractText()
            contents.append(PageContent(page_number, page_text))
    return contents

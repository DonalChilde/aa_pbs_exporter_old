...
def text_pages(
    file_path: Path, start_page: int = 0, end_page: int = -1
) -> Generator[TextPage, None, None]:
    with open(file_path, "rb") as file:
        read_pdf = PdfFileReader(file)
        number_of_pages = read_pdf.getNumPages()
        if number_of_pages is None:
            raise ValueError(f"No pages found for {file_path}")
        if end_page == -1 or end_page > number_of_pages:
            end_page = int(number_of_pages)
        if start_page > end_page:
            raise ValueError(
                f"Requested start page {start_page} is bigger than the total pages {end_page}. Cannot count backwards."
            )
        for page_number in range(start_page, end_page):
            page = read_pdf.getPage(page_number)
            page_text_ = page.extractText()
            yield TextPage(page_number, page_text_)


def get_page_content2(
    file_path: Path,
    start_range: int = 0,
    end_range: int = -1,
    ...
    """
    # This function left as an example for a future feature
    # accept functions that can test for start, valid, and end
    # e.g. test for page number to start, test for a regex on page, test for regex
    # signaling end of output.
    # def start(input:PageText)->bool
    ...
    with open(file_path, "rb") as f:
        ...
        for x in range(start_range, end_range):
            ...
            page_text_ = page.extractText()
            if not start_flag:
                if start_filter(x, page_text_):
                    retval.append(TextPage(x, page_text_))
                    start_flag = True
                    continue
                continue
            if end_filter(x, page_text_):
                retval.append(TextPage(x, page_text_))
                break
            if content_filter(x, page_text_):
                retval.append(TextPage(x, page_text_))
                continue
    return retval
